...
import { moduleNameForType } from "./fileUtils";
...
export const resolveTsTypeForMapType = (
    ...
    if (IType.isReference(mapType.keyType)) {
        ...
        if (ITypeDefinition.isEnum(keyTypeDefinition)) {
            if (mapType.keyType.reference.package === baseType.package) {
                return `{ ${maybeReadonly}[key in ${mapType.keyType.reference.name}]?: ${resolvedValueType} }`;
            } else {
                const moduleSpecifier = moduleNameForType(mapType.keyType.reference);
                return `{ ${maybeReadonly}[key in ${moduleSpecifier}.${mapType.keyType.reference.name}]?: ${resolvedValueType} }`;
            }
        } else if (
            ...
            if (mapType.keyType.reference.package === baseType.package) {
                ...
                const moduleSpecifier = moduleNameForType(mapType.keyType.reference);
                return `{ ${maybeReadonly}[key: ${moduleSpecifier}.I${mapType.keyType.reference.name}]: ${resolvedValueType} }`;
            }
        }
    }
...
export const resolveTsTypeForReferenceType = (
    ...
    const isSameModule = referencedType.package === baseType.package;
    const moduleSpecifier = moduleNameForType(referencedType);
